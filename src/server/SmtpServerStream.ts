...
export class SmtpStream extends Writable {
   ...
   * @param on_binary_data Gets called when we're done receiving binary data.
   * @param on_command Gets called when there is a command.
   * @param on_data Gets called when there is new data.
   * @param on_data_max_reached when the data reached max.
   * @param on_line when a line is received.
   */
  public constructor(
  ...
  protected async _handle_data_write(): Promise<void> {
    ...
    if (this._data_max_length && this._buffer.length > this._data_max_length) {
      await this.on_data_max_reached();
      return;
    }
