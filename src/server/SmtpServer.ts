...
export class SmtpServer extends EventEmitter {
   ...
   */
  public constructor(
    public readonly config: SmtpServerConfig,
    public readonly hostname: string = "0.0.0.0",
    public readonly plain_port: number = 25,
    public readonly secure_port: number = 465,
    public readonly backlog: number = 500,
    public readonly timeout: number = 5 * 1000 * 60
  ) {
    super();
    ...
    if (this.config.size_limit) {
      this.capabilities.push(
        new SmtpCapability(
          SmtpCapabilityType.Size,
          this.config.size_limit.toString()
        )
      );
    }
    ...
    if (this.config.feature_enabled(SmtpServerFeatureFlag.Auth)) {
      this.capabilities.push(
        new SmtpCapability(SmtpCapabilityType.Auth, [
          SmtpAuthType.PLAIN,
  ...
  protected _event_connection(
    ...
    smtp_socket.on("close", () =>
      this.emit("client_disconnected", secure, smtp_socket)
    );
    smtp_socket.set_timeout(this.timeout);

    const session: SmtpServerSession = new SmtpServerSession();
    const connection: SmtpServerConnection = new SmtpServerConnection(
      this,
      smtp_socket,
      session
